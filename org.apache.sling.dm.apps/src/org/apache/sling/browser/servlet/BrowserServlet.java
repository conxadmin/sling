...
public class BrowserServlet extends SlingAllMethodsServlet implements ManagedService {
			...
			if (isModeChildren) {
				...
				jsonWriter.array();
					jsonWriter.object().key("label").value(path.equals(ROOT_PATH) ? "jcr:root" : browserResource.getNode().getName());
						jsonWriter.key("path").value("/"+tenant.getPID()+"/"+browserResource.getPath());
						...
						Iterator<Resource> it = browserResource.getChildren().iterator();
						if (it.hasNext()) {
	...
	private  void listChildren(final Iterator<Resource> it, final JSONWriter jsonWriter, final MimeTypeService mtService) throws RepositoryException {
		if (it.hasNext()) {
			try {
				jsonWriter.array();
				while (it.hasNext()) {
					BrowserResource resource = (BrowserResource) it.next();
					jsonWriter.object().key("label").value(resource.getNode().getName());
					jsonWriter.key("path").value("/"+tenant.getPID()+"/"+resource.getPath());
