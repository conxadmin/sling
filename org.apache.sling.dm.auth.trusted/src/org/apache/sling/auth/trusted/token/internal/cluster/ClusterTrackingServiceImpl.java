...
import org.apache.sling.auth.trusted.token.api.cluster.ClusterServer;
import org.apache.sling.auth.trusted.token.api.cluster.ClusterTrackingService;
import org.apache.sling.auth.trusted.token.api.cluster.ClusterUser;
import org.apache.sling.auth.trusted.token.api.cluster.cache.ClusterServerImpl;
import org.apache.sling.auth.trusted.token.api.cluster.cache.ClusterUserImpl;
...
import org.apache.sling.auth.trusted.token.util.osgi.EventUtils;
...
/**
 * The ClusterTrackingService, maintains an entry for the active server and
 * tracks active users with a cluster replicated shared cache.
 */
public class ClusterTrackingServiceImpl implements ClusterTrackingService, Runnable {
	 ...
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(ClusterTrackingServiceImpl.class);
	/**
	...
	/**
	 * Activate the service, getting the id of the jvm instance and register the
	 * instance.
	 *
	 * @param ctx
	 * @throws Exception
	...
	protected void deactivate() throws Exception {
		try {
			...
			LOGGER.info(
					"Cluster Tacking Cache has already been disposed, the server registration will timeout on other nodes :{}",
					e.getMessage());
	...
	/**
	 * Track a user, called from a filter, this will update the central cache
	 * for the cookie and set the cookie if not present.
	 *
	 * @param request
	 *            the http request oject.
	 * @param response
	 *            the http response object which will, if there is no cookie
	 *            present, and the response is not committed, have the cookie
	 *            set.
	 */
	public void trackClusterUser(HttpServletRequest request, HttpServletResponse response) {
		...
		if (!tracking && !response.isCommitted()) {
			// the tracking cookie is the a sha1 hash of the thread, the server
			// startup id and
			// time
			...
			// we *do not* track cookies the first time, to avoid DOS on the
			// cookie store.
			// pingTracking(trackingCookie, remoteUser);
		}
	 ...
	 */
	private void pingTracking(String trackingCookie, String remoteUser) throws PingRemoteTrackingFailedException {
	...
	private void pingRemoteTracking(String trackingCookie, String remoteUser) throws PingRemoteTrackingFailedException {
		...
		if (clusterServer == null) {
			throw new PingRemoteTrackingFailedException("Server at " + trackingCookie + " not alive ");
		}
	...
	private void pingInstance() {
		if (isActive) {
			if (!isReady) {
				do {
					updateServerNumber();
					getServerCache().put(serverId, new ClusterServerImpl(serverId, serverNumber, thisSecureUrl));
					try {
						Thread.sleep(1000);
				...
				try {
					...
					// prevent excessive logging in edge case where cache ==
					// null;
	...
	private boolean checkServerNumber() {
		...
		for (ClusterServer server : servers) {
			if (server.getServerNumber() == serverNumber && !serverId.equals(server.getServerId())) {
				return false;
			}
		}
		return true;
	 ...
	 * {@inheritDoc}
	 *
	 * Invoked as a task by the Sling Scheduler, once every 5 minutes to update
	 * the last time the server was registered in the cluster cache.
	 *
